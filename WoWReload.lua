...
function WoWReload_OnLoad(self)
    self:RegisterEvent("ASSET_LOADED")
...
function WoWReload_OnEvent(self, event, ...)
    ...
    elseif event == "ASSET_LOADED" then
        local success, path = ...
        local text = WoWReloadScrollFrameText:GetText()
        if strlen(text) > 0 then
            WoWReloadScrollFrameText:SetText(text .. "\n" .. path)
        else
            WoWReloadScrollFrameText:SetText(path)
        end
    end
end
